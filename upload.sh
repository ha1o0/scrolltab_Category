...
DevelopVersionString=`grep -E 'version.develop.*=' scrolltab_Category.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' scrolltab_Category.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' scrolltab_Category.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' scrolltab_Category.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" scrolltab_Category.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' scrolltab_Category.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" scrolltab_Category.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' scrolltab_Category.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" scrolltab_Category.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' scrolltab_Category.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' scrolltab_Category.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" scrolltab_Category.podspec
...
pod repo push ${Repo} scrolltab_Category.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
